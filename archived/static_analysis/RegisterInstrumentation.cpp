...
const bool FILTER_INVERSE =
    false;  // false = keep ONLY instructions in filter
...
class RegisterPopSnippet : public Snippet {
 public:
  std::string fn;

  RegisterPopSnippet(std::string name) { fn = name; }
  ...
  virtual bool generate(Point* pt, Buffer& buf) {
    ...
    Label sstack = a.newLabel();
    Label ret = a.newLabel();

    uint64_t stack_ptr = 0x100000000;

    a.cmp(asmjit::x86::r11, imm(9));
    a.jz(sstack);

    printf("Function : %s\n", fn.c_str());

    if (!strcmp(fn.c_str(), "foo")) {
      a.cmp(asmjit::x86::r12, asmjit::x86::qword_ptr(asmjit::x86::rsp));
      a.jnz(abort);
      a.jmp(ret);
    } else {
      a.cmp(asmjit::x86::r15, asmjit::x86::qword_ptr(asmjit::x86::rsp));
      a.jnz(abort);
      a.jmp(ret);
    }
...
class RegisterPushSnippet : public Snippet {
 public:
  std::string fn;

  RegisterPushSnippet(std::string name) { fn = name; }
  ...
  virtual bool generate(Point* pt, Buffer& buf) {
    ...
    if (!strcmp(fn.c_str(), "foo")) {
      a.mov(asmjit::x86::r12, asmjit::x86::qword_ptr(asmjit::x86::rsp));
      a.jmp(end);
    } else {
      a.mov(asmjit::x86::r15, asmjit::x86::qword_ptr(asmjit::x86::rsp));
      a.jmp(end);
    }
...
void handleInstruction(void* addr, Instruction::Ptr iptr, PatchBlock* block,
                       PatchFunction* func) {
  ...
  printf("  instruction at %lx: %s\n", (unsigned long)addr,
         iptr->format((Address)addr).c_str());

  if (id == e_ret_near) {

    Snippet::Ptr snippet =
        RegisterPopSnippet::create(new RegisterPopSnippet(""));
    ...
    Snippet::Ptr snippet =
        RegisterPushSnippet::create(new RegisterPushSnippet(""));
...
void handleFunction(BPatch_function* function, const char* name) {
  ...
  */
  ...
  if (strcmp(name, "f") == 0 || strcmp(name, "g") == 0) {
    ...
    mainMgr->findPoints(Scope(PatchAPI::convert(function)), Point::FuncEntry,
                        back_inserter(pts));

    BPatch_snippet* nullExpr = new BPatch_nullExpr();
    Snippet::Ptr snippet = PatchAPI::convert(nullExpr);

    for (vector<Point*>::iterator iter = pts.begin(); iter != pts.end();
         ++iter) {
      Point* pt = *iter;
      pt->pushBack(snippet);
    }
  ...
  std::string fn = strcmp(name, "foo") ? "bar" : "foo";

  Snippet::Ptr snippet =
      RegisterPushSnippet::create(new RegisterPushSnippet(fn));
  ...
  mainMgr->findPoints(Scope(PatchAPI::convert(function)), Point::FuncExit,
                      back_inserter(pts));

  snippet = RegisterPopSnippet::create(new RegisterPopSnippet(fn));
...
int main(int argc, char* argv[]) {
  ...
  ((BPatch_binaryEdit*)app)->writeFile("register_mutant");
  printf("Done.\n");
