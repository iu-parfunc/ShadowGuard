...
struct InterProcSummary {
    ...
    bool skipInstrumentation() {
        return !writesMemory && !adjustSP;
    }

    bool operator!=(const InterProcSummary & rhs) const {
        return (writesMemory != rhs.writesMemory) ||
               (adjustSP != rhs.adjustSP);
    }
};
...
void CalculateFunctionSummary(CodeObject *co, Function* f) {
    ...
    for (auto b : f->blocks()) {
        ...
        for (auto const& ins : insns) {
            ...
            bool isCall = (ins.second.getCategory() == Dyninst::InstructionAPI::c_CallInsn);
            if (isCall) continue;
        ...
        for (auto e: b->targets()) {
            ...
            if (co->cs()->linkage().find(e->trg()->start()) != co->cs()->linkage().end()) {
            ...
            if (callee) s.callees.insert(callee);
        }
    }
}
...
void FunctionSummaryPropagation(CodeObject* co) {
    for (auto f: co->funcs()) {
        ...
        ips.writesMemory = s.writesMemory || s.containsPLTCall || s.containsUnknownCF;
...
void PerformStackAnalysis(Function *f) {
    ...
    AssignmentConverter converter(true /* cache results*/, true /* use stack analysis*/);
                ...
                for (auto a: assigns) {
                    ...
                    switch (loc.type()) {
                        case Absloc::Stack:
                            ...
                            if (loc.off() >= 0) s.writesMemory = true;
                            break;
                        case Absloc::Unknown:
...
int main(int argc, char **argv) {
    ...
    printf("Analysis 1: only skip leaf function that does not write memory or adjust SP\n");
    int count = 0;
    for (auto f: co->funcs()) {
        FuncSummary &s = funcSummary[f];
    ...
    printf("Analysis 2: skip leaf function that do not write unknown memory or adjust SP\n");
    for (auto f: co->funcs()) {
    ...
    printf("Analysis 3: skip function that itself and its callee do not write unknown memory or adjust SP\n");
    count = 0;
    for (auto f: co->funcs()) {
